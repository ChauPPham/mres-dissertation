...
\begin{titlingpage}
	\begin{center}
		...
		\begin{minipage}[!h]{.89\linewidth}\large 
			...
			Signature
			\vskip 0.5em
			\includegraphics[width=3cm]{Signature} \vskip 0.2em Pham Phu Chau \vskip 0.2em
			September 7, 2023
		\end{minipage}
		\vskip 4cm
...
Several issues arise with the use of the series of questions on lifetime income gamble to capture risk preference of mothers when they make investment on their children, namely relevance, stability and temporal properties. 
...
\parskip 0cm
\begin{equation}
P(c=j)=P(\log\underline{\theta}_j<x<\log\bar{\theta}_j)=\Phi\Big({\log(\bar{\theta}_j)-\mu-b_q\over\sqrt{\sigma_x^2+\sigma_{eq}^2+\sigma_{\kappa q}^2}}\Big)-\Phi\Big({\log(\underline{\theta}_j)-\mu-b_q\over\sqrt{\sigma_x^2+\sigma_{eq}^2+\sigma_{\kappa q}^2}}\Big) 
\label{eq:A-likelihood-constant-univariate}	
\end{equation}
where $\mu$ and $\sigma_x$ are respectively the mean and standard deviation of true log risk tolerance. $\sigma_{eq}^2$ is variance in an individual's transitory response error for a particular wave and question type $q$ while $\sigma_{\kappa q}^2$ is variance in an individual's persistent response error for question type $q$. $\bar \theta_j$ and $\underline{\theta}_j$ are upper and lower bounds on the coefficient of risk tolerance. $b_q$ common bias across individuals to questions of type $q$. The transitory response error in wave $w$ for question type $q$ is $\epsilon_{wq}$ which is assumed to be purely random and independent of an individual's true risk tolerance or any other attributes, i.e., $\epsilon_{wq}\sim N(0,\sigma^2_{eq})$. The coefficient of risk aversion, $\gamma$, is then imputed using the conditional expectation that an individual belongs to a certain risk category $j$,

\begin{equation}
\mathbb E(\gamma|c=j)=\exp\Big(-\mu+{\sigma_x^2\over 2}\Big){\Phi\Big({\log(\bar{\theta}_j)-\mu-b_q+\sigma_x^2\over \sqrt{\sigma_x^2+\sigma_{eq}^2+\sigma_{\kappa q}^2}}\Big)-\Phi\Big({\log(\underline{\theta}_j)-\mu-b_q+\sigma_x^2\over \sqrt{\sigma_x^2+\sigma_{eq}^2+\sigma_{\kappa q}^2}}\Big)\over \Phi\Big({\log(\bar{\theta}_j)-\mu-b_q\over \sqrt{\sigma_x^2+\sigma_{eq}^2+\sigma_{\kappa q}^2}}\Big)-\Phi\Big({\log(\underline{\theta}_j)-\mu-b_q\over \sqrt{\sigma_x^2+\sigma_{eq}^2+\sigma_{\kappa q}^2}}\Big)}
...
\end{equation}
where $\vec{\Phi}$ is the bivariate normal cumulative distribution function, $\bar N_{jq}=(\log \bar \theta_j-\mu-b_q)/\sigma_q,\bar N_{kq'}=(\log \bar \theta_k-\mu-b_{q'})/\sigma_{q'}, \underline N_{jq}=(\log \underline \theta_j-\mu-b_q)/\sigma_q, \underline N_{kq'}=(\log \underline \theta_k-\mu-b_{q'})/\sigma_{q'}$, $\sigma_q=\sqrt{\sigma_x^2+\sigma_{eq}^2+\sigma_{\kappa q}^2}$, and $\rho$ is the correlation between answers across the two waves. If respondents participated in more than two waves, the form of \eqref{eq:A-likelihood-constant-bivariate} alters accordingly to accommodate multivariate normal distribution.

\subsection{Time-varying risk preference}
Under the assumption of time-variant risk preference, we augment our estimation with both time-invariant and time-variant characteristics of participants. The former comprise gender, race, parents' years of schooling, an indicator of whether the respondent lived with both biological parents at age 14, and AFQT score (2006 revised edition). The latter consist of area of residence, marital status, age, educational level, log of income, and the number of weeks worked in the previous year by respondent and respondent's spouse.\footnote{Educational level is allowed to vary with time to capture the potential endogeneity of education, i.e., more educated individuals are less risk averse but less risk averse individuals may also choose to obtain more education.} To further control for fluctuations in macroeconomic condition at the time questions on risk preference were asked, we also include the unemployment rate and index of consumer sentiment (ICS) as covariates. Under this assumption, the probability of being in category $j$ for an individual $i$ who answered in only one wave is
\begin{equation}
...
\end{equation}
where $X_{it}$ denotes the set of characteristics of the respondents at the time of the survey, $t$. We can modify \eqref{eq:A-likelihood-floating-univariate} in the same spirit to formulate the likelihood of respondents who participated in more than one wave. 
