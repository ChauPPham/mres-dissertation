...
where $\mu$ and $\sigma_x$ are respectively the mean and standard deviation of log risk tolerance for a certain wave of response. $\bar \theta_j$ and $\underline{\theta}_j$ are upper and lower bounds on the coefficient of risk tolerance. $b$ denotes status quo bias and $\sigma_\xi$ standard deviation of response error. $b$ takes value of -0.21 while $\sigma_x$ takes value of 0.76. \footnote{These values are computed for the Health and Retirement Study (HRS) capitalizing on the difference in responses to two different versions of the risk preference questions. For more information, see \citet{kimball2009risk} and their accompanying appendix.} Imputed values of $\gamma$ are shown in \Cref{table:3-imputed-risk}.
