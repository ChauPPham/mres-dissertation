...
for (i in ed_cols){
  for (j in 1:nrow(education)){
    if (!is.na(pool2[j,i])) {
      if (pool2[j, i] == 97) pool2[j,i] = -99
    }
  }
}

education$max_ed <- apply(pool2[, ed_cols], 1, max, na.rm = TRUE)

pool1 <- inner_join(amb, education, by = "prim_key")
pool2 <- inner_join(merge, education, by = "prim_key")
